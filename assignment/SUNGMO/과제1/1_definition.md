...
**소프트웨어 (ROS2 기반)** : 최근 로봇 표준 프레임워크인 **ROS2**를 사용해, 기능을 하나의 펌웨어가 아니라 **역할별 노드**로 나누고 **토픽**으로 메시지를 주고받도록 구성한다.
※ 본 과제는 별도의 개발(구현) 과정을 포함하지 않으나, 참고용 구현 예시 코드를 함께 제출한다(`ros2_ws/src/line_follower_agv/`).

| 노드 | 역할 | 요구 | 구독 토픽 | 발행 토픽 |
|---|---|---|---|---|
...
| `data_logger_node` | 이동 데이터 CSV 저장 | ③ | `/odom`, `/line_error` 등 | (CSV 파일) |
| `mqtt_bridge_node` | 텔레메트리 MQTT 실시간 전송 | ④ | `/odom`, `/line_error` 등 | (MQTT 토픽) |

토픽 흐름: `sensor_node → /line_error → controller_node → /cmd_vel → (베이스 드라이버 → 모터)` 이고, `/odom`(엔코더+IMU 기반 위치·속도)은 `data_logger_node`(CSV)와 `mqtt_bridge_node`(MQTT)가 함께 구독한다. 모터 구동·`/odom` 생성은 베이스 드라이버(micro-ROS / ros2_control)가 담당한다. (동작 흐름은 위 "자율주행 동작 순서도" 참조)
